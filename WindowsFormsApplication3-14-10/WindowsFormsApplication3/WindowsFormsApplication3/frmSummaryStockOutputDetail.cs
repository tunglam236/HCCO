        ...
        private void frmSummaryStockOutputDetail_Load(object sender, EventArgs e)
        {
            ...
            if (type == "1")
            {
                ...
                foreach (var item in s.ToList())
                {
                    ...
                    dr[1] = item.ProductId;
                    dr[2] = item.Code; ;
                    dr[3] = item.ProductName.ToUpper();
                    dr[4] = item.Quantity;
                    dr[5] = string.Format("{0:0,0}", item.Price);
                    dr[6] = item.Discount > 999 ? string.Format("{0:0,0}", item.Discount) : item.Discount.ToString();
                    dr[7] = string.Format("{0:0,0}", (item.Price - item.Discount) * item.Quantity);
                    dt.Rows.Add(dr);
            ...
            else
            {
                ...
                foreach (var item in s.ToList())
                {
                    ...
                    dr[4] = item.Quantity;
                    dr[5] = string.Format("{0:0,0}", item.Price);
                    dr[6] = string.Format("{0:0,0}", item.Quantity * item.Price);
