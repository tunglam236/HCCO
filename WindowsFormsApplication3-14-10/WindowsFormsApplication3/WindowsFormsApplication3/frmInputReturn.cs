...
namespace WindowsFormsApplication3
{
    public partial class frmInputReturn : Office2007Form
    {
        ...
        private void txtOutputCode_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (txtOutputCode.Text.Trim() != "")
                {
                    ...
                    var sp = db.sp_getBillCode_Return(WindowsFormsApplication3.Form1.branch_id.ToString(), txtOutputCode.Text.Trim());
                    int i = 0;
                    foreach (var d in sp.ToList())
                    {
                        ...
                        dr[2] = d.ProductCode;
                        dr[3] = d.ProductName;
                        dr[4] = d.Quantity.ToString();
                        dr[5] = string.Format("{0:0,0}", d.Price);
                        dr[6] = d.DiscountPercent.ToString() + "%";
                        dr[7] = d.Discount.ToString();
                        dr[8] = string.Format("{0:0,0}", d.Total);
                        dr[9] = d.IsCombo.ToString();
                        dr[10] = d.Note.ToString();
                        dtTemp.Rows.Add(dr);
                        i++;
                    }
                ...
                try
                {
                    ...
                    if (d.Count() > 0)
                    {
                        if (d.FirstOrDefault().Price > 0)
                        {
                            ...
                            dr[2] = d.FirstOrDefault().ProductName;
                            dr[3] = currrent_quantity + 1;
                            dr[4] = string.Format("{0:0,0}", d.FirstOrDefault().Price);
                            dr[5] = string.Format("{0:0,0}", (currrent_quantity + 1) * d.FirstOrDefault().Price);
                            dr[6] = d.FirstOrDefault().IsCombo;
                            dtTemp.Rows.Add(dr);
        ...
        private void dgvProductNew_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                if (dgvProductNew.CurrentCell.ColumnIndex == 4)
                {
