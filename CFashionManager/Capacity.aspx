<%@ Page Title="Danh mục dung tích" Language="C#" MasterPageFile="~/Admin.master" AutoEventWireup="true" CodeFile="Capacity.aspx.cs" Inherits="Capacity" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cph" Runat="Server">
   ...
   <div class="row">
      ...
      <section class="">
   <div class="pull-left">
      <h3 style="margin:5px; font-weight:bold;"><i class="fa fa-sitemap"></i> Danh mục dung tích</h3>
   </div>
   <div style="text-align:right; padding-bottom:10px;">
    <a href="#" onclick="addColor();" class="btn btn-sm btn-success" data-toggle="modal" data-target="#addColor"><i class="glyphicon glyphicon glyphicon-edit"></i> Thêm dung tích</a>
      <div class="dropdown">
      <button class="btn btn-sm btn-danger dropdown-toggle " data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><i class="glyphicon glyphicon-th"></i> Hiển thị</button>
        <ul class="dropdown-menu ul-column" style="margin-left:-65px;">
            <li><a href="#" class="small toggle-vis" data-column="1" tabIndex="-1"><label><input type="checkbox" checked/>&nbsp;Mã dung tích</label> </a></li>
            <li><a href="#" class="small toggle-vis" data-column="2" tabIndex="-1"><label><input type="checkbox" checked/>&nbsp;Tên dung tích</label> </a></li>
                ...
                <table id="example2" class="table hover cell-border" style="min-width:915px; font-size:12px;">
                  <thead>
                     <tr>
                        ...
                        <th class="bg-th">Mã dung tích</th>
                        <th class="bg-th">Tên dung tích</th>
...
<div class="modal fade" id="addColor" role="dialog">
            ...
            <div class="modal-content" style="font-size: 12px;">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title center">
                        ...
                        <span id="lb">THÊM DUNG TÍCH</span>
                    </h4>
                </div>
                <div class="modal-body">
                    <div class="row" style="padding: 5px;">
                        ...
                        <div class="col-md-3">
                         ...
                         <div class="col-md-3">Tên dung tích<br />
    ...
    <script type="text/javascript">
        function updateColor_modal(id, branchtypeId, branchTypeName, code, name,note) {
            ...
            $('#removeColor').show();
            document.getElementById('lb').innerText = 'CẬP NHẬT DUNG TÍCH';
        }
        function addColor() {
            $("#addColor").modal({ show: false });
            $('#hdColorId').val('');
            ...
            $('#txtNote').val('');
            $('#removeColor').hide();
            document.getElementById('lb').innerText = 'THÊM DUNG TÍCH';
        }
        function saveChanges() {
            ...
            if (id == '') {
                ...
                if (code != '' && name != '') {
                    $.ajax({
                        type: 'POST',
                        url: '/Command.aspx/insertCapacity',
                        data: '{"branchTypeId":"' + branchType + '","code":"' + code + '","name":"' + name + '","note":"'+note+'"}',
                        contentType: 'application/json; charset=utf-8',
                        dataType: 'json',
                        success: function (data) {
                            if (data.d._content == '1') {
                                showAlert('Đã thêm dung tích [' + name + ']');
                ...
                else showAlert('Nhập mã và tên dung tích');
            }
            else {
                updateChanges();
            }
        ...
        function updateChanges() {
            ...
            if (code != '' && name != '') {
                $.ajax({
                    type: 'POST',
                    url: '/Command.aspx/updateCapacity',
                    data: '{"id":"' + id + '","branchTypeId":"' + branchType + '","code":"' + code + '","name":"' + name + '","note":"'+note+'"}',
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'json',
                    success: function (data) {
                        if (data.d._content == '1') {

                            showAlert('Đã cập nhật dung tích [' + name + ']');
        ...
        function deleteColor() {
            ...
            if (confirm("Bạn chắc chắn muốn xóa [" + name + "] ?") == true) {
                if (id != '') {
                    $.ajax({
                        type: 'POST',
                        url: '/Command.aspx/deleteCapacity',
                        data: '{"id":"' + id + '"}',
                        contentType: 'application/json; charset=utf-8',
                        dataType: 'json',
                        success: function (data) {
                            if (data.d._content == '1') {
                                ...
                                $('#txtNote').val('');
                                $('#delete' + id).remove();
                                showAlert('Đã xóa dung tích [' + name + ']');
                                $("#addColor").modal('hide');
                            }
                            else
                                showAlert(data.d._mess);
                ...
                else showAlert('Chọn dung tích muốn xóa');
            }
        }
    </script>
</asp:Content>
