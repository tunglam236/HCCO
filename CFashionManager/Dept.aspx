<%@ Page Title="Quản lý bộ phận, phòng ban" Language="C#" MasterPageFile="~/Admin.master" AutoEventWireup="true" CodeFile="Dept.aspx.cs" Inherits="Dept" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cph" Runat="Server">
   ...
   <div class="row">
      ...
      <section class="">
   <div class="pull-left">
      <h3 style="margin:5px; font-weight:bold;"><i class="fa fa-star"></i> Danh mục bộ phận, phòng ban</h3>
   </div>
   <div style="text-align:right; padding-bottom:10px;">
    <a href="#" onclick="addDept();" class="btn btn-sm btn-success" data-toggle="modal" data-target="#addDept"><i class="glyphicon glyphicon glyphicon-edit"></i> Thêm bộ phận</a>
      <div class="dropdown">
      <button class="btn btn-sm btn-danger dropdown-toggle " data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><i class="glyphicon glyphicon-th"></i> Hiển thị</button>
        <ul class="dropdown-menu ul-column" style="margin-left:-65px;">
            <li><a href="#" class="small toggle-vis" data-column="1" tabIndex="-1"><label><input type="checkbox" checked/>&nbsp;Mã bộ phận</label> </a></li>
            <li><a href="#" class="small toggle-vis" data-column="2" tabIndex="-1"><label><input type="checkbox" checked/>&nbsp;Tên bộ phận</label> </a></li>
                ...
                <table id="example2" class="table hover cell-border" style="min-width:650px; font-size:12px;">
                  <thead>
                     <tr>
                        ...
                        <th class="bg-th">Mã bộ phận</th>
                        <th class="bg-th">Tên bộ phận</th>
...
<div class="modal fade" id="addDept" role="dialog">
                ...
                <div class="modal-body">
                    <div class="row" style="padding: 5px;">
                        <div class="col-md-4">
                         ...
                         <div class="col-md-4">Tên bộ phận<br />
    ...
    <script type="text/javascript">
        function update_modal(id, code, name, des) {
            ...
            $('#removeDept').show();
            document.getElementById('lb').innerText = 'CẬP NHẬT BỘ PHẬN';
        }
        function addDept() {
            $("#addDept").modal({ show: false });
            $('#hdDeptId').val('');
            ...
            $('#txtDescription').val('');
            $('#removeDept').hide();
            document.getElementById('lb').innerText = 'THÊM BỘ PHẬN';
        }
        function saveChanges() {
            ...
            if (id == '') {
                ...
                if (code != '' && name != '') {
                    $.ajax({
                        type: 'POST',
                        url: '/Command.aspx/insertDept',
                        data: '{"code":"' + code + '","name":"' + name + '","des":"'+des+'"}',
                        contentType: 'application/json; charset=utf-8',
                        dataType: 'json',
                        success: function (data) {
                            if (data.d._content == '1') {
                                showAlert('Đã thêm bộ phận [' + name + ']');
                                if (ckload) {
                                    setTimeout(function () {
                ...
                else showAlert('Nhập mã và tên bộ phận');
            }
            else {
                updateChanges();
            }
        ...
        function updateChanges() {
            ...
            if (code != '' && name != '') {
                $.ajax({
                    type: 'POST',
                    url: '/Command.aspx/updateDept',
                    data: '{"id":"' + id + '","code":"' + code + '","name":"' + name + '","des":"'+des+'"}',
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'json',
                    success: function (data) {
                        if (data.d._content == '1') {

                            showAlert('Đã cập nhật bộ phận [' + name + ']');
        ...
        function deleteDept() {
            ...
            if (confirm("Bạn chắc chắn muốn xóa [" + name + "] ?") == true) {
                ...
                else showAlert('Chọn bộ phận muốn xóa');
            }
        }
    </script>
</asp:Content>
